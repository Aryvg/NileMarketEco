...
async function fetchUsersFromBackend() {
  try {
    ...
    const res = await fetch('https://nilemarketbackend.onrender.com/users', { headers });
...
async function checkDashboardRole() {
  try {
    const res = await fetch('https://nilemarketbackend.onrender.com/refresh/status', {
      method: 'GET',
      credentials: 'include'
    });
    if (res.ok) {
...
async function fetchProductsFromBackend() {
  try {
    const token = localStorage.getItem('accessToken');
    const headers = token ? { 'Authorization': 'Bearer ' + token } : {};
    const res = await fetch('https://nilemarketbackend.onrender.com/products', { headers });
...
async function fetchOrdersAndUsers() {
  try {
    ...
    const token = localStorage.getItem('accessToken');
    const headers = token ? { 'Authorization': 'Bearer ' + token } : {};
    const usersRes = await fetch('https://nilemarketbackend.onrender.com/users', { headers });
    let usersData = [];
    if (usersRes.ok) {
    ...
    const ordersRes = await fetch('https://nilemarketbackend.onrender.com/orders?all=1', { headers });
    let ordersData = [];
    if (ordersRes.ok) {
...
`;
  ...
  row.querySelector(".deleteBtn").onclick=()=>{
    ...
    document.getElementById('confirmDeleteYes').onclick = async function() {
      ...
      try {
        ...
        const res = await fetch(`https://nilemarketbackend.onrender.com/users/${u._id}`, {
          method: 'DELETE',
          headers
        });
  ...
  roleSel.onchange = async e => {
    ...
    async function ensureAccessToken() {
      ...
      try {
        const r = await fetch('https://nilemarketbackend.onrender.com/refresh', { method: 'GET', credentials: 'include' });
    ...
    if (u._id) {
      try {
        ...
        const res = await fetch(`https://nilemarketbackend.onrender.com/users/${u._id}/role`, {
          method: 'PUT',
          headers,
...
fetch('https://nilemarketbackend.onrender.com/products', {
  method: 'POST',
  body: formData,
...
products.filter(p => p.name.toLowerCase().includes(filter.toLowerCase())).slice().reverse()
  .forEach((p) => {
    ...
    if (imgSrc && !/^https?:\/\//.test(imgSrc)) {
      imgSrc = `https://nilemarketbackend.onrender.com/${imgSrc}`;
    }
    box.innerHTML = `
      <img src="${imgSrc}">
  ...
  const accessToken = localStorage.getItem('accessToken');
  try {
    const res = await fetch('https://nilemarketbackend.onrender.com/products', {
      method: 'PUT',
      body: fd,
      headers: accessToken ? { 'Authorization': 'Bearer ' + accessToken } : {}
    });
