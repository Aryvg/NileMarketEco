...
export async function searchProducts(query) {
  try {
    let token = null;
    try {
      const r = await fetch('https://nilemarketbackend.onrender.com/refresh', { credentials: 'include' });
      if (r.ok) {
    ...
    const response = await fetch(`https://nilemarketbackend.onrender.com/products/search?name=${encodeURIComponent(query)}`, { headers });
    if (!response.ok) {
